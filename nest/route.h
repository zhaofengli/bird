...
struct nexthop {
  ...
  u32 label[0];
  byte seg6s;               /* Number of all SRv6 segments */
  byte seg6_encap_mode;     /* SRv6 encapsulation mode (SEG6_IPTUN_MODE_...) */
  byte seg6_flags;          /* SRv6 route flags */
  u32 seg6_hmac_keyid;      /* SRv6 HMAC KEYID (optional, non-zero) */
  ip6_addr seg6[0];         /* SRv6 segments, in reverse order */
};
...
#define NEXTHOP_MAX_SIZE (sizeof(struct nexthop) + sizeof(u32)*MPLS_MAX_LABEL_STACK + sizeof(ip6_addr)*SEG6_MAX_SEGMENT_LIST)

static inline size_t nexthop_size(const struct nexthop *nh)
{ return sizeof(struct nexthop) + sizeof(u32)*nh->labels + sizeof(ip6_addr)*nh->seg6s; }
...
void rta_init(void);
static inline size_t rta_size(const rta *a) { return sizeof(rta) + sizeof(u32)*a->nh.labels + sizeof(ip6_addr)*a->nh.seg6s; }
#define RTA_MAX_SIZE (sizeof(rta) + sizeof(u32)*MPLS_MAX_LABEL_STACK + sizeof(ip6_addr)*SEG6_MAX_SEGMENT_LIST)
